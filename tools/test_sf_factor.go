...
func main() {
	...
	factors := []struct {
		...
		{"Eclipse Year", 346.6201},       // 交点年
		{"Besselian Year", 365.2422},     // 贝塞尔年
		{"Calendar Year", 365.0},         // 日历年
		{"Leap Year Avg", 365.25},        // 闰年平均
		{"Solar Fire 1", 365.249924},     // 从Sp-Sp案例推断
		{"Solar Fire 2", 365.250054},     // 从Tr-Sa案例推断
	...
	for _, targetHour := range targetHours {
		...
		// diff_hours = (sf_prog_jd - our_prog_jd) * 24
		// sf_prog_jd = natalJD + (testJD-natalJD)/sf_factor
		// our_prog_jd = natalJD + (testJD-natalJD)/JulianYear
		// diff_hours = ((testJD-natalJD)/sf_factor - (testJD-natalJD)/JulianYear) * 24
		// diff_hours = (testJD-natalJD) * (1/sf_factor - 1/JulianYear) * 24
		// sf_factor = 1 / (1/JulianYear + diff_hours/(24*(testJD-natalJD)))
	...
	for _, tc := range testCases {
		...
		// 反向计算Solar Fire使用的因子
		// sf_jd = natal_jd + (event_jd - natal_jd) / sf_factor
		// sf_factor = (event_jd - natal_jd) / (sf_jd - natal_jd)
