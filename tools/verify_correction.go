...
func main() {
	...
	if err != nil {
		...
		fmt.Printf("  位置差异: %.4f°\n", sweph.MinDistance(sfLon, tradLon))
	}
	
	// 太阳弧计算验证
	fmt.Println("\n=== 太阳弧验证 ===")
	
	saOffset, err := progressions.SolarArcOffset(natalJD, testJD)
	if err != nil {
		...
		// 验证太阳弧位置
		saLon, saSpeed, err := progressions.CalcSolarArcLongitude(planet, natalJD, testJD)
		if err != nil {
			fmt.Printf("太阳弧位置计算错误: %v\n", err)
		} else {
			natalLon, _, _ := chart.CalcPlanetLongitude(planet, natalJD)
			expectedSALon := sweph.NormalizeDegrees(natalLon + saOffset)
			fmt.Printf("  太阳弧位置: %.4f°\n", saLon)
			fmt.Printf("  期望位置: %.4f°\n", expectedSALon)
			fmt.Printf("  位置差异: %.4f°\n", sweph.MinDistance(saLon, expectedSALon))
		}
	}
	...
	fmt.Println("\n=== 总结 ===")
	fmt.Printf("修正前时间差异: %.1f小时\n", timeDiffHours)
	if abs(timeDiffHours) < 1.0 {
		fmt.Println("✓ 时间差异在可接受范围内 (< 1小时)")
	} else if abs(timeDiffHours) < 5.0 {
